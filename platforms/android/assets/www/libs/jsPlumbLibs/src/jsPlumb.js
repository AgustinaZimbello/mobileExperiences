				...
				if (draggable) {
					if (jpcl.isDragSupported(element) && !jpcl.isAlreadyDraggable(element)) {
						...
						options[dragEvent] = _wrap(options[dragEvent], function() { 
							...
							//adding colission properties
							/*var collisions = $(element).collision( "#main", { relative: "collider", obstacleData: "odata", colliderData: "cdata", directionData: "ddata", as: "<div/>" } );
						...
						options[stopEvent] = _wrap(options[stopEvent], function(e) {
							...
							//After dragging, if the element dragged is dropped into #main we convert that element into a marker
							/*if(inMain){
								this.remove(); //Delete the element
								//Catching mouse position corrdinates
								...
								mark._icon.children[0].id = copyId+"ep";
								
								addMarkerToList(copyId,copyClass,latlng.lat,latlng.lng,idEp);
							...
							//If empty == true it means that no CONTENT is inside east panel
							/*var empty = true;

							//Checking empty variable
							$("#east-panel ul").children().each(function(){
								if (this.className == "ui-droppable panel-content elemento copy leaflet-marker-icon ui-draggable _jsPlumb_endpoint_anchor_"){ 
									empty = false; 
								};
							});

							//If empty == true, I move ALL arrows to #main.
							if(empty){
