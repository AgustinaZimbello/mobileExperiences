...
REDIPS.drag = (function () {
	...
	var	init,						// initialization
		...
		mark = {action: 'deny',
				cname: 'mark',
				exception: []},
		style = {borderEnabled : 'none',	// (string) border style for enabled elements
		...
		if (drop !== false) {
			...
			// normal row move
				...
				// insert target row to source location and delete source row (if row is not cloned)
					...
					// delete emptyRow flag to the source row (needed in case when switching last row from t2 to table1)
